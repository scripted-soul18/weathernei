...
export const WeatherPredictionMobile: React.FC<WeatherPredictionMobileProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col justify-between bg-[#070E1A] text-slate-100 min-h-[calc(100vh-2rem)]">
      {/* 1. Mobile Top Header */}
      <header className="px-3.5 py-2.5 bg-[#091220] border-b border-slate-800 flex items-center justify-between gap-2 sticky top-0 z-40 backdrop-blur-md">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <button
            onClick={onBackToNavigation}
            className="p-2 rounded-xl bg-slate-900 border border-slate-800 text-emerald-400 hover:text-emerald-300 active:scale-95 transition-all shrink-0"
          ...
          <div
            ...
            className="flex-1 min-w-0 cursor-pointer bg-slate-950/80 px-2.5 py-1.5 rounded-xl border border-slate-800 flex items-center gap-1.5 hover:border-slate-700"
          >
            <MapPin className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
            <div className="text-left truncate text-xs font-bold text-white leading-tight">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            className="p-2 rounded-xl bg-slate-900 border border-slate-800 text-amber-400 relative active:scale-95"
          ...
          <button
            onClick={onRefresh}
            disabled={isLoading}
            className="p-2 rounded-xl bg-slate-900 border border-slate-800 text-blue-400 active:scale-95"
            title="Refresh"
          >
      ...
      <div className="px-3 pt-2 pb-1.5 bg-[#070E1A] border-b border-slate-800/80">
        <div className="grid grid-cols-3 gap-1 p-1 rounded-2xl bg-[#0B1526] border border-slate-800">
          <button
      ...
      <main className="flex-1 p-3 space-y-3 overflow-y-auto pb-4">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-3 animate-fadeIn">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="p-2.5 rounded-2xl bg-[#0B1526] border border-slate-800">
                <div className="text-[10px] text-slate-400">Hazard Risk</div>
                <div className={`text-base font-black font-mono mt-0.5 ${
                  currentRisk === 'VERY HIGH' ? 'text-rose-400' : currentRisk === 'HIGH' ? 'text-amber-400' : currentRisk === 'MODERATE' ? 'text-yellow-400' : 'text-emerald-400'
                }`}>
                  {currentRisk}
                </div>
              </div>
            ...
            {weatherData ? (
              ...
              <div className="p-6 rounded-2xl bg-[#0B1526] border border-slate-800 text-center text-xs text-slate-400">
        ...
        {activeTab === 'map' && (
          <div className="space-y-3 animate-fadeIn">
            <div className="h-[380px] sm:h-[440px] rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
              <InteractiveMap
                latitude={currentLat}
            ...
            <div className="p-3 rounded-2xl bg-[#0B1526] border border-slate-800 text-xs text-slate-300 flex items-center justify-between">
              <span>Elevation: {predictionData?.terrain?.elevation || 350}m</span>
              <span>Slope: {predictionData?.terrain?.slope || 15}°</span>
              <span className="text-cyan-400 font-semibold font-mono">DEM ACTIVE</span>
            </div>
          </div>
        )}
      ...
      {showSearchModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-start justify-center p-4 pt-12">
          <div className="w-full max-w-sm rounded-3xl bg-slate-900 border border-slate-800 p-4 space-y-3 shadow-2xl">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <span className="text-xs font-bold text-white">Search Corridors &amp; Locations</span>
              <button
      ...
      {/* 4. Mobile Bottom Navigation Bar matching Image 2 */}
      <nav className="px-2 py-2 bg-[#060C16] border-t border-slate-800/80 sticky bottom-0 z-40">
        <div className="flex items-center justify-around relative">
          {/* Trips */}
          <button
            onClick={onBackToNavigation}
            className="flex flex-col items-center gap-1 py-1 px-3 text-slate-400 hover:text-slate-200 transition-colors"
          >
          ...
          <button
            onClick={() => setActiveTab('overview')}
            className="flex flex-col items-center gap-1 py-1 px-3 text-emerald-400 font-bold transition-all relative"
          >
          ...
          <button
            onClick={onBackToNavigation}
            className="flex flex-col items-center gap-1 py-1 px-3 text-slate-400 hover:text-slate-200 relative transition-colors"
          >
